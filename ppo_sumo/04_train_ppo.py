...
GAMMA = 0.99
GAE_LAMBDA = 0.95 # specifies the lambda factor in the advantage estimator. The value of 0.95 was used in the PPO paper.
...
if __name__ == "__main__":
    ...
    save_path = "/home/chenkehan/RESEARCH/codes/hands-on-book/chapter19"
    ...
    with ptan.common.utils.RewardTracker(writer) as tracker:
        '''
        exp here is history in ExperienceSource and there is:
        Experience = namedtuple('Experience', ['state', 'action', 'reward', 'done'])
        history.append(Experience(state=state, action=action, reward=r, done=is_done))
        history is a deque!
        '''
        for step_idx, exp in enumerate(exp_source): # won't stop!
            rewards_steps = exp_source.pop_rewards_steps() # rewards_steps is sth like: [(total_rewards, total_steps), (total_rewards, total_steps), ...]
            if rewards_steps:
                rewards, steps = zip(*rewards_steps)
                writer.add_scalar("episode_steps", np.mean(steps), step_idx)
                tracker.reward(np.mean(rewards), step_idx) # for tensorboard
